...
void optra(double **a, unsigned int m, unsigned int n, double **c, unsigned int k, unsigned int *ic1, unsigned int *ic2,
	   unsigned int *nc, double *an1, double *an2, unsigned int *ncp, double *d, unsigned int *itran,
	   unsigned int *live, unsigned int *indx);

void qtran(double **a,unsigned int m,unsigned int n,double **c,unsigned int k,unsigned int *ic1,unsigned int *ic2,unsigned int *nc,
	   double *an1,double *an2,unsigned int *ncp,double *d,unsigned int *itran,unsigned int *indx);

void kmeans(double **a, unsigned int m, unsigned int n, double **c, unsigned int k, unsigned int *ic1,unsigned int *nc,
	   unsigned int iter,double *wss,unsigned int *ifault)
{
...
/* Update cluster centres to be the average of prints contained within them. */
	...
	for (ij = 0;ij < iter; ++ij) {
		...
		if (indx == m) 	ij=iter;
			...
			qtran(a, m, n, c, k, ic1, ic2, nc, an1, an2, ncp, d,
			      itran, &indx);
      			if (k==2) ij = iter;/* k=2: no need to re-enter the optimal transfer stage*/
			else {	/* ncp has to be set to 0 before entering optra. */				for (l=0;l<k; ++l) ncp[l] = 0;
			}
		}
	} 
