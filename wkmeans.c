...
/* Adapted into weighted k-means by Geoffrey Thompson, Ames, IA 12/31/16           */



void woptra(double **a, unsigned int m, unsigned int n, double *weights, double **c, unsigned int k,
	    unsigned int *ic1, unsigned int *ic2,
	    double *nc, unsigned int *ncp, double *d, unsigned int *itran,
	    unsigned int *live, unsigned int *indx);

void wqtran(double **a,unsigned int m,unsigned int n,double *weights, double **c,unsigned int k,
	    unsigned int *ic1,unsigned int *ic2,double *nc,
	    unsigned int *ncp,double *d,unsigned int *itran,unsigned int *indx);

void wkmeans(double **a, unsigned int m, unsigned int n, double *weights, double **c, unsigned int k,
	     unsigned int *ic1,double *nc,
	     unsigned int iter,double *wss,int *ifault)
{
  ...
  *ifault = 0;
  ...
  for (i = 0; i < m; ++i) {
    for (j = 0; j < n; ++j)
      if(weights[i]>0.) c[ic1[i]][j] += a[i][j] * weights[i];
  }
  ...
  for (j = 0; j < n; ++j) {
    ...
    for (i = 0; i < m; ++i) {
      da = a[i][j] - c[ic1[i]][j];
      if(weights[i]>0.) wss[ic1[i]] += weights[i] * da * da;
    }
  }
  FREE_VECTOR(ic2);
  //FREE_VECTOR(an1);
	   ...
	   unsigned int *live, unsigned int *indx)
{
  ...
  for (i = 0; i < m; ++i) {
    ...
    if ( (*indx) >= m) return;
    /* should be *indx == m but did it this way to be slightly more robust to programmer error */
	    ...
	    unsigned int *ncp,double *d,unsigned int *itran,unsigned int *indx)
{
  ...
  while (iflag==0) {
	...
	if (nc[l1] > weights[i]) {
      ...
      if (icoun>=m) return;
      /* should be icoun == m but did it this way to be slightly more robust to programmer error */
