/**
   ...
   A non-parallel C implementation of k-means|| (or scalable k-means++).
   ...
   EDIT: added stochastic acceptance sampling, unbiased randint()
   03/14/19
...
void wkmeans(double **a, unsigned int m, unsigned int n, double *weights,
	     double **c, unsigned int k, unsigned int *ic1,double *nc,
...
void wkmeanspp(double **x, unsigned int n, unsigned int p, unsigned int k, double *neighbors, double **centers){
	...
	if(k == 1){
	  ...
	  y = WSample(n, neighbors); //maybe prob
	  for(i = 0; i < p; i++)
	    centers[0][i] = x[y][i];
	  ...
	  for(l = 0; l < (k-1); l++){
		...
		if (distMin > D[i]) j = p; /*go to the end of the loop*/
	      ...
	      if(distMin < D[i])
		D[i] = distMin;
	      if (Dmax < neighbors[i]*D[i])
		Dmax = neighbors[i]*D[i];			
	    ...
	    for(i = 0; i < n; i++)
	      prob[i] = (neighbors[i] > 0.) ? D[i]*neighbors[i] : 0.;
	    
	    
	    
	    /* Choose one center with probability defined from above */
	    
	    ans = WMaxSample(n, prob, Dmax);
	    z[chosen] = ans;
	    D[ans] = 0.;
	    for(j = 0; j < p; j++)
